...
% Second subsurface position, a perturbation to xyz to explore around C-DOG
...
% Rows where none of three entries are NaNs
...
if plt == 1
    ...
    hold on
    p(2)=plot(d.t,st,'k--','LineWidth',lwidth);
    hold off
    ...
    hold on
    p(4)=plot(d.t,movmean(differ*tmulti{1,1},3600,'omitnan'),'y','LineWidth',lwidth);
    hold off
    % Must set ticks every n hours
    datetick('x','HH')
    legend({'pred-obs','1-hr average'},'Location','NorthWest')
    ...
    if pval >= pthresh
        text(ah(2).XLim(2)-0.125*abs(ah(2).XLim(2)-ah(2).XLim(1)),0.9*ah(2).YLim(1),...
             sprintf('ACCEPTED'))
    else
        text(ah(2).XLim(2)-0.125*abs(ah(2).XLim(2)-ah(2).XLim(1)),0.9*ah(2).YLim(1),...
             sprintf('REJECTED'))
    ...
    % some more cosmetics
    set(ttt{1},'FontSize',7.5)
    set(ttt{2},'FontSize',7.5)
    set(ah,'FontSize',7.5)
    delete([t(1) t(2) t(3) t(4)])
    movev(ah(2),0.05)
    movev(ah(3:5),0.06);
    movev(tt,-0.07);
    %delete(tt)
